"""
Low-level dump of both databases: table schemas, event/message counts from
runs.db, and the raw learned LinUCB weights from policy.db.

Usage: python inspect_db.py [--runs-db runs.db] [--policy-db policy.db]
"""
...
def show_policy(db="policy.db"):
    """Print each LinUCB arm's b vector (reward-weighted feature sums) and A diagonal (visit counts, roughly)."""
    con = sqlite3.connect(db)
    cur = con.cursor()
